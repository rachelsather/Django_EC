...
@login_required #verification; prevents unauthroized access
def profile(request):
...
def myfeed(request):
    comment_count_list = []
    like_count_list =[]
    posts = Post.objects.filter(username=request.user).order_by('-date_posted') #filter because there's more than one
    for p in posts:
...
def friends(request):
    ...
    # to get elgible profiles = exclude the user, their existing friends, and friend requests already sent
    ...
    if request.method == 'POST' and request.POST.get("receive_requests"):
        ...
        for sender in senders:
            ...
            # create a relationship object to access the sender's user id to add the friends list of the user
